...
def reset_empty_kb():
    ...
    qa_agent.ENSEMBLES.pop("integration-session", None)  # 只清该会话知识库，不复建 agent（快）
...
class TestEmptyKBBehavior:
    def test_chitchat_answered(self):
        """闲聊不应调检索，直接回答"""
        reset_empty_kb()
        ans = qa_agent.ask("你好", [], session_id="integration-session")
        assert isinstance(ans, str) and len(ans) > 0

    def test_knowledge_question_guides_upload(self):
        """知识类问题在空库时应引导上传文档"""
        reset_empty_kb()
        ans = qa_agent.ask("刹车片多久换一次？", [], session_id="integration-session")
...
class TestAfterUpload:
    def test_ask_after_upload(self):
        ...
        qa_agent.rebuild([doc], session_id="integration-session")
        ans = qa_agent.ask("刹车片多久换一次？", [], session_id="integration-session")
...
class TestMemory:
    def test_multi_turn_remembers_context(self):
        ...
        reset_empty_kb()
        h = []
        a1 = qa_agent.ask("我叫小明", h, session_id="integration-session")
        h += [{"role": "user", "content": "我叫小明"}, {"role": "assistant", "content": a1}]
        a2 = qa_agent.ask("我叫什么名字？", h, session_id="integration-session")
